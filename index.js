...
async function registerCommands() {
  const commands = [
    ...
    .setDescription(
      "Configure the thread message upon opening a thread (Admin only)",
    )
    .addStringOption((option) =>
    option
    .setName("text")
    .setDescription("embed description to be sent upon thread creation"),
    )
    .setDefaultMemberPermissions("0"),
    new SlashCommandBuilder()
...
client.on("threadCreate", async (thread) => {
  ...
  if (config?.createdTag) {
    try {
      ...
      console.error(
        `failed to add tag: ${config?.createdTag} for ${thread.id} upon it's creation:\n${e}`,
      );
    }
  }
...
client.on("interactionCreate", async (interaction) => {
  if (
    ...
    interaction.customId === "select_role_management_menu"
  ) {
  ...
  if (
    interaction.isRoleSelectMenu() &&
    interaction.customId === "role_management_select_roles_menu"
  ) {
...
async function handleConfigRoleManagementCommand(interaction) {
  ...
  if (!theRole) {
    ...
    .setDescription("select the role to manage!")
    .setTimestamp();
...
async function handleSelectedRoleMenuInteraction(interaction, interrole) {
  ...
  ? `You selected the role: ${role.name}`
  : "Role not found.";
  let components = [];
  if (role) {
    ...
    .setPlaceholder(
      "Select few Roles to give current role management power over them!",
    )
    .setMinValues(0)
    .setMaxValues(25); // we are limited to 25 by discord api
...
async function handleConfigCommand(interaction) {
  ...
  if (excludedTags)
    config.excludedTags = excludedTags.split(",").map((tag) => tag.trim());
...
/**
 * Get the highest role position from a member's roles
...
/**
 * Get all roles that a member can manage based on Discord hierarchy
 * @param {GuildMember} member
 * @param {Guild} guild
 * @returns {Set<string>} Set of role IDs
 */
function getRolesManageableByHierarchy(member, guild) {
  const manageableRoles = new Set();
  const highestPosition = getHighestRolePosition(member);

  guild.roles.cache.forEach((role) => {
    // Can manage if role is below their highest role and not @everyone
    if (role.position < highestPosition && role.id !== guild.id) {
      manageableRoles.add(role.id);
    }
  });
...
function getAllManageableRolesByRoles(roles) {
  ...
  for (const managerRoleId of roles) {
    const manageableRoles = getManageableRolesByRole(managerRoleId);
    if (manageableRoles) {
...
async function handleRoleCommand(message, args, action) {
  ...
  if (!targetMember) {
    return basicEmbedReply(
      message,
      "Please specify a member (mention them or reply to their message).",
    );
  }
  ...
  if (role === undefined) {
    return basicEmbedReply(
      message,
      `Could not find a role with the name or ID: \`${roleIdentifier}\`.`,
      0x690202,
    );
  }
  ...
  const hasManageRolesPermission = member.permissions.has(
    PermissionFlagsBits.ManageRoles,
  );

  let allowedRolesToManage = new Set();

  // Admins can manage everything
  if (member.permissions.has(PermissionFlagsBits.Administrator)) {
    guild.roles.cache.forEach((role) => allowedRolesToManage.add(role.id));
  }
  // Users with Manage Roles permission can manage roles below their highest role
  else if (hasManageRolesPermission) {
    allowedRolesToManage = getRolesManageableByHierarchy(member, guild);
  }
  // Otherwise, use the custom role permission system
  else if (authorRoles.some((role) => allowedRoles.includes(role))) {
    allowedRolesToManage = getAllManageableRolesByRoles(authorRoles);
  } else {
    return basicEmbedReply(
      message,
  ...
  if (!allowedRolesToManage.has(role.id)) {
    return basicEmbedReply(
      message,
      `You are not permitted to manage the **${role.name}** role. ${hasManageRolesPermission ? "This role is at or above your highest role." : ""}`,
    );
  }
  ...
  try {
    if (action === "add") {
      if (targetMember.roles.cache.has(role.id)) {
        return basicEmbedReply(
          message,
          `${targetMember.displayName} already has the **${role.name}** role.`,
        );
      }
      await targetMember.roles.add(role);
      await basicEmbedReply(
        message,
        `Successfully added the **${role.name}** role to ${targetMember.displayName}.`,
      );
    } else if (action === "remove") {
      if (!targetMember.roles.cache.has(role.id)) {
        return basicEmbedReply(
          message,
          `${targetMember.displayName} does not have the **${role.name}** role.`,
        );
      }
      await targetMember.roles.remove(role);
      await basicEmbedReply(
        message,
        `Successfully removed the **${role.name}** role from ${targetMember.displayName}.`,
      );
    }
  } catch (error) {
    ...
    basicEmbedReply(
      message,
      `I was unable to ${action} the role. Make sure my role is higher than the **${role.name}** role in the server roles or that it exists!`,
      0x690202,
    );
  }
}

const addRoleCommand = (message, args, config) =>
handleRoleCommand(message, args, "add", config);
const removeRoleCommand = (message, args, config) =>
handleRoleCommand(message, args, "remove", config);
